...
void MainWindow::createActions()
{
    ...
    /*Free-form Membrane Signal Slot*/
...
PolygenMesh* MainWindow::_buildPolygenMesh(std::string name) {

    PolygenMesh* newMesh = new PolygenMesh();
...
void MainWindow::generateTetraMeshbyTetgen() {
    ...
    meshOperator->tetMeshGeneration_singleSurfaceMesh(patch, tetMesh, "Ya5.0");
...
void MainWindow::split_Show_TET_Mesh() {
    ...
    PolygenMesh* tetModel = nullptr;
    for (GLKPOSITION pos = polygenMeshList.GetHeadPosition(); pos != nullptr;) {
        PolygenMesh* polygenMesh = (PolygenMesh*)polygenMeshList.GetNext(pos);
        if (polygenMesh->getModelName() == "TET") tetModel = polygenMesh;
    }
    ...
    for (GLKPOSITION Pos = patch->GetFaceList().GetHeadPosition(); Pos;) {
        ...
        if (face->show_innerFace_split) {
            for (int i = 0; i < 3; i++) {
                face->GetEdgeRecordPtr(i)->show_innerEdge_split = true;
            }
        }
    }
