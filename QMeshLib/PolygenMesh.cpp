...
void PolygenMesh::_buildDrawShadeList(bool bVertexNormalShading)
{
   ...
   isTransparent = false;
    ...
    if (isTransparent){
        ...
        glEnable(GL_DEPTH_TEST);
        glDepthMask(GL_FALSE);              //to see other objs not only at nearest z value
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glEnable(GL_BLEND);
        glEnable(GL_CULL_FACE);    //not show back face
    }
    ...
    for(Pos=meshList.GetHeadPosition();Pos!=NULL;meshIndex++) {
        ...
        for(PosFace=(mesh->GetFaceList()).GetHeadPosition();PosFace!=NULL;)
        {
            ...
            if (isTransparent) {
                if (drawIdx==1) { rr = 0.8f; gg = bb = 0.2f; glColor3f(rr, gg, bb);}
                else glColor4f(rr, gg, bb, 0.4);
            }
    ...
    for(Pos=meshList.GetHeadPosition();Pos!=NULL;meshIndex++) {
        ...
        for(PosEdge=(mesh->GetEdgeList()).GetHeadPosition();PosEdge!=NULL;) {
           ...
           /* if (edgeColor){
            ...
            {
                rr = gg = bb = 0.0;
                bool _boundaryNode = false;


                for (GLKPOSITION facePos = (edge->GetFaceList()).GetHeadPosition(); facePos != NULL;) {
                    QMeshFace* _face = (QMeshFace*)((edge->GetFaceList()).GetNext(facePos));
                    if (_face->GetLeftTetra() == NULL || _face->GetRightTetra() == NULL)
                    {
                        //boundary face
                        _boundaryNode = true;
            ...
            {
                if (!edge->show_innerEdge_split && !isTransparent) {
                    if (!edge->GetStartPoint()->isChamberNode || !edge->GetEndPoint()->isChamberNode)
                        continue;
                }
            }
           /* if (edge->selected)
...
void PolygenMesh::_buildDrawNodeList()
{
    ...
    for(Pos=meshList.GetHeadPosition();Pos!=NULL;meshIndex++) {
        ...
        for(PosNode=(mesh->GetNodeList()).GetHeadPosition();PosNode!=NULL;) {
            ...
            { 
                //rr = 0.6; gg = 0.2; bb = 0.2; 
                rr = 1.0; gg = 0.0; bb = 0.2;
            }



            if(node->selectedRigid) { rr = 0.6; gg = 0.6; bb = 0.2; }
            if (node->isTerminalChecking) { rr = 0.2; gg = 0.6; bb = 0.2; }
            if (node->isCollided) { rr = 0.2; gg = 0.0; bb = 0.8; }
            ...
            if(drawIdx == 3)
            {
                ...
                for (GLKPOSITION facePos = (node->GetFaceList()).GetHeadPosition(); facePos != NULL;) {
                    QMeshFace* _face = (QMeshFace*)((node->GetFaceList()).GetNext(facePos));
                    if (_face->GetLeftTetra() == NULL || _face->GetRightTetra() == NULL)
                    {
                        //boundary face
                        _boundaryNode = true;
                ...
                else if (node->isCollided_env == true)
                {
                    gg = 1.0;
                }
...
void PolygenMesh::_buildDrawProfileList()
{
    ...
    //    if (this->getModelName() != "TET") continue;
    ...
    //        if (node->isCollided) {
    //            std::cout << "collided point!" << std::endl;
    //            Eigen::Vector3d pp; node->GetCoord3D(pp[0], pp[1], pp[2]);
    //            std::cout << pp << std::endl << std::endl;

    //            glVertex3d(pp[0], pp[1], pp[2]);
    //            QMeshFace* _collidedFacePtr = node->_collidedFacePtr;
    //            Eigen::Matrix3d facepos;
    //            for (int i = 0; i < 3; i++) {
    //                _collidedFacePtr->GetNodeRecordPtr(i)->GetCoord3D(facepos(i, 0), facepos(i, 1), facepos(i, 2));
    //            }
    //            Eigen::Vector3d nextP = Eigen::Vector3d::Zero();
    //            for (int i = 0; i < 3; i++) {
    //                for (int j = 0; j < 3; j++) {
    //                    nextP(i) += facepos(j, i) * node->_barycentricCoord[j];
    //                }
    //            }
    //            //std::cout << nextP << std::endl << std::endl;
    //            glVertex3d(nextP(0), nextP(1), nextP(2));

    //            //Eigen::Vector3d nextP = node->_rayOrigin + node->_collidedT * node->_rayDir;
    //            ////std::cout << node->_rayOrigin << std::endl << std::endl;
    //            ////std::cout << node->_collidedT << std::endl << std::endl;
    //            //glVertex3d(nextP(0), nextP(1), nextP(2));
    //            //std::cout << nextP << std::endl << std::endl;
    //            ////glVertex3d(node->_rayOrigin(0), node->_rayOrigin(1), node->_rayOrigin(2));
