...
public:
	...
	bool _intersect_triangle(RAY R, Eigen::Vector3d A, Eigen::Vector3d B, Eigen::Vector3d C, double& t, double& u, double& v, Eigen::Vector3d& N);
	
	//for debug in collision detection
	GLKPOSITION startIndexfromPos = NULL;
	int startIndexfromPos_int = 0;

	int collidedPntNum = -1;		//self collision point number
	int collidedPntNum_env = -1;	//collision with environment

	//mannequin
	int freeformChamberIdx = -1;	//from 1 to 4.
