...
QMeshTetra* QMeshFace::GetAnotherTetra(QMeshTetra* _oneTetra)
{
	//pLeftTetra

	if (_oneTetra == pLeftTetra)
		return pRightTetra;
	else if (_oneTetra == pRightTetra)
		return pLeftTetra;
	else
		return NULL;
}
...
std::set<QMeshFace*> QMeshFace::GetNeighbourFacesOnSurface(void)
{
	std::set<QMeshFace*> _neighFaceSet;
	QMeshEdge* _edge;
	int eNum = this->GetEdgeNum();
	for (int j = 0; j < eNum; j++) {
		_edge = this->GetEdgeRecordPtr(j);
		QMeshFace* _edgeNeighbourFace = NULL;
		for (GLKPOSITION Pos = _edge->GetFaceList().GetHeadPosition(); Pos != NULL; ) {
			QMeshFace* face = (QMeshFace*)(_edge->GetFaceList().GetNext(Pos));
			if (face->inner == true) { continue; }
			else if (face == this) continue;
			else _edgeNeighbourFace = face;
		}
		if (_edgeNeighbourFace == NULL) qDebug("ERROR Code 2...");
		_neighFaceSet.insert(_edgeNeighbourFace);
	}

	return _neighFaceSet;
}